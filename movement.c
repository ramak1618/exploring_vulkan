...
/*
void translate_camera(float* restrict viewmat, const float* restrict dvec) {
    float V[4] = {
        viewmat[12] - viewmat[0]*dvec[0] - viewmat[4]*dvec[1] - viewmat[8]*dvec[2],
        viewmat[13] - viewmat[1]*dvec[0] - viewmat[5]*dvec[1] - viewmat[9]*dvec[2],
        viewmat[14] - viewmat[2]*dvec[0] - viewmat[6]*dvec[1] - viewmat[10]*dvec[2],
        1.f,
    };

    memcpy(&viewmat[12], V, 4*4); // HOPEFULLY SIMD-ified (hence enforced 16-byte copy instead of 12-byte copy)
}

void populate_new_translation_vector(float* restrict newview, const float* restrict oldview) {
    // Hopefully SIMD-ified (henced enforced 4x4 * 4x1 instead of 3x3 * 3x1)
    double campos[4] = {
        oldview[0] * oldview[12] + oldview[1] * oldview[13] + oldview[2] * oldview[14],
        oldview[4] * oldview[12] + oldview[5] * oldview[13] + oldview[6] * oldview[14],
        oldview[8] * oldview[12] + oldview[9] * oldview[13] + oldview[10] * oldview[14],
        1.f,
    };
    newview[12] = newview[0]*campos[0] + newview[4]*campos[1] + newview[8]*campos[2]  + newview[3]*campos[3];
    newview[13] = newview[1]*campos[0] + newview[5]*campos[1] + newview[9]*campos[2]  + newview[7]*campos[3];
    newview[14] = newview[2]*campos[0] + newview[6]*campos[1] + newview[10]*campos[2] + newview[11]*campos[3];
    newview[15] = newview[3]*campos[0] + newview[7]*campos[1] + newview[11]*campos[2] + newview[15]*campos[3];
}

void update_camera(float* viewmat, enum camera_movement movement_type, float velocity, float delta_time) {
    switch(movement_type) {
        case MOVEMENT_FORWARD: {
            float dvec[3] = { 
                viewmat[2] * velocity * delta_time,
                viewmat[6] * velocity * delta_time,
                viewmat[10] * velocity * delta_time
            };
            translate_camera(viewmat, dvec);
            return;
        }
        ...
        case MOVEMENT_LEFT: {
            float dvec[3] = {
                -viewmat[0] * velocity * delta_time,
                -viewmat[4] * velocity * delta_time,
                -viewmat[8] * velocity * delta_time,
            };
            translate_camera(viewmat, dvec);
            return;
        }
        ...
        case MOVEMENT_UP: {
            float dvec[3] = {
                -viewmat[1] * velocity * delta_time,
                -viewmat[5] * velocity * delta_time,
                -viewmat[9] * velocity * delta_time,
            };
            translate_camera(viewmat, dvec);
            return;
        }
        ...
        case MOVEMENT_TURNLEFT: {
            float cos_wt = cos(velocity * delta_time);
            float sin_wt = sin(velocity * delta_time);

            float newview[16] = {
                viewmat[0] * cos_wt + viewmat[2]  * sin_wt, viewmat[1], 0.f, 0.f,
                viewmat[4] * cos_wt + viewmat[6]  * sin_wt, viewmat[5], 0.f, 0.f,
                viewmat[8] * cos_wt + viewmat[10] * sin_wt, viewmat[9], 0.f, 0.f,
                                   0.f,                        0.f,     0.f, 1.f,  
            };

            float mag = sqrt( viewmat[0] * viewmat[0] + viewmat[4] * viewmat[4] + viewmat[8] * viewmat[8]);
            viewmat[0] /= mag;
            viewmat[4] /= mag;
            viewmat[8] /= mag;

            newview[2] = newview[4] * newview[9] - newview[8] * newview[5];
            newview[6] = newview[8] * newview[1] - newview[0] * newview[9];
            newview[10] = newview[0] * newview[5] - newview[4] * newview[1];

            populate_new_translation_vector(newview, viewmat);
            memcpy(viewmat, newview, 16*4);
            return;
        }
